...
function render() {
  ...
  text('gate-heading',g.ready ? 'This candidate satisfies the configured release gate.' : 'Release blocked. Resolve the conditions below.');
  ...
  populateControls();renderEvidence();renderResources();renderApprovals();
  if(currentView==='planning')renderPlan();if(currentView==='lifecycle')renderLifecycle();text('ledger-head',short(state.checkpoint.hash));
